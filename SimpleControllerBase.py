...
class SimpleControllerBase(SimpleControlSurface):
  ...
  def _create_midi_element(self, ctrl, ch = 0, is_cc = True):
    """
    Returns a MIDI element (either CC or note), e.g., for controlling lights on
    a MIDI device. Typical usage:
  ...
  def _register_slider(self, callback, ctrl, ch = 0, is_cc = True):
    """
    Attaches the given callback to a MIDI element (either CC or note). The
    callback is invoked when a MIDI event comes in; it takes a single argument,
    a 7-bit MIDI value. Returns the MIDI element.
    """
  ...
  def _register_trigger(self, callback, ctrl, ch = 0, is_cc = True):
    """
    Attaches the given callback to a momentary MIDI element (either CC or note);
    the callback takes no arguments, and it is invoked when the MIDI element
    emits a nonzero value. Returns the MIDI element.
    """
